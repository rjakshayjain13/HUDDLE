...
for f in files_txt:
    ...
    for row in reader:
        if temp>1:
            if row[0] != '' and row[1] != '':
                data.setdefault(row[0] + row[1], []).append(list([weeknum]) + list([int(row[5])]) + list([int(row[17])]) + list([float(row[19])]))
                label.setdefault(row[0]+row[1], []).append(list([weeknum]) + list([int(row[38])]))
            elif row[0] == '' and row[1] != '':
                data.setdefault(row[1], []).append(list([weeknum]) + list([int(row[5])]) + list([int(row[17])]) + list([float(row[19])]))
                label.setdefault(row[1], []).append(list([weeknum]) + list([int(row[38])]))
            elif row[1] == '' and row[0] != '':
                data.setdefault(row[0], []).append(list([weeknum]) + list([int(row[5])]) + list([int(row[17])]) + list([float(row[19])]))
...
wb = xw.Workbook("KNN_Output.xlsx")
...
ws1.write(1,0,'KNN Results:',format)
...
ws2.write(1,0,'KNN Predictions:',format)
